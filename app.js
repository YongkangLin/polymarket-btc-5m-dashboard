...
const ACTIVE_BACKTEST_KEY = "portfolio_ranked_a65q25e2p50_queue_distmom_strict_then_a4070_sharedcap10";
const ACTIVE_BACKTEST_VALUE = `candidate:${ACTIVE_BACKTEST_KEY}`;
const ACTIVE_PAPER_EDGE_ID = "portfolio_ranked_a65q25e2p50_queue_distmom_strict_then_a4070_sharedcap10_native";
...
const LIVE_RENDER_MAX_SOURCE_ROWS_PER_LINE = 500;
const LIVE_RENDER_MIN_POINTS_PER_LINE = 80;
const LIVE_RENDER_MAX_POINTS_PER_LINE = 240;
const LIVE_RENDER_POINTS_PER_PIXEL = 0.35;
const LIVE_AUX_VERSION_THROTTLE_MS = 250;
const LIVE_CHART_SCHEMA_VERSION = "paper-live-v54-strict-distmom";
...
function liveTickStorageKeysForMarket(market) {
  const canonical = polymarketWindowKeyForMarket(market)
    || browserWindowKeyForMarket(market)
    || paperGraphKey(market);
  return canonical ? [canonical] : [];
}

function liveTickReadStorageKeysForMarket(market) {
  return [...new Set([
    polymarketWindowKeyForMarket(market),
    browserWindowKeyForMarket(market),
    paperGraphKey(market),
...
function liveTickPointsForMarket(market) {
  if (!market) return [];
  const keys = new Set(liveTickReadStorageKeysForMarket(market));
  if (!keys.size) return [];
  [...paperGraphMarkets(), ...state.livePersistedMarkets.values()].forEach((candidate) => {
    ...
    liveTickReadStorageKeysForMarket(candidate).forEach((key) => keys.add(key));
  });
  const start = marketWindowStartUnix(market);
...
function renderBacktestSelects() {
  ...
  const coverage = backtestCoverageSummary();
  byId("marketFilter").innerHTML = [
    ["all", `All validated markets (${fmt.format(allMarkets.length)})`],
  ...
  const aggregateLabel = state.marketFilter === "bought"
    ...
    : state.marketFilter === "no_action"
      ...
      : `All validated markets (${fmt.format(filteredMarkets.length)} charted)`;
  byId("backtestMarket").innerHTML = [
    `<option value="${BACKTEST_ALL_VALUE}">${escapeHtml(`${aggregateLabel} | ${coverage.text}`)}</option>`,
...
function renderStrategyPanels() {
  ...
  const fills = activeSummary.algorithm_fills ?? activeSummary.fills ?? activeSummary.filled_markets ?? signalRows().length;
  ...
  byId("backtestStrategy").innerHTML = [
    ...
    strategyCell(
      "Result",
      `${fmt.format(quoteMarkets)} quote markets | ${fmt.format(fills)} fills | ${formatSignedMoney(totalPnl)} P&L | ${formatPercent(totalRoi)} return | ${coverage.text}`,
    ),
  ].join("");
...
function marketDecisionSummary(row, market, isSignal) {
  ...
  return {
    ...
    totalResult: `${fmt.format(buyCount)} fills | ${formatPercent(totalRoi)} return | ${coverage.text}`,
    headline,
    result,
    reason,
    signals: [
...
function backtestPerformanceSummary() {
  const activeSummary = state.workflow.active_backtest?.summary || state.workflow.backtest?.summary || {};
  const coverage = backtestCoverageSummary();
  const cleanMarkets = coverage.validated;
  const buyCount = metricNumber(activeSummary.algorithm_fills ?? activeSummary.fills ?? activeSummary.filled_markets ?? activeSummary.signals) ?? 0;
...
function backtestCoverageSummary() {
  const activeSummary = state.workflow?.active_backtest?.summary || state.workflow?.backtest?.summary || {};
  const q = state.workflow?.data_quality || {};
  const manifest = backtestSeriesManifestSummary();
  const marketCount = marketRows().length;
  const validated = metricNumber(
    activeSummary.validated_replay_markets
      ?? q.validated_replay_markets
      ?? activeSummary.complete_series_markets
      ?? q.complete_series_markets
      ?? marketCount
      ?? activeSummary.algorithm_admitted_markets,
  ) ?? 0;
  const charted = metricNumber(
    manifest.markets
      ?? activeSummary.charted_series_markets
      ?? activeSummary.chart_complete_series_markets
      ?? activeSummary.complete_series_markets
      ?? q.charted_series_markets
      ?? q.complete_series_markets
      ?? marketCount,
  ) ?? 0;
  const excluded = metricNumber(
    activeSummary.chart_excluded_incomplete_series_markets
      ?? activeSummary.excluded_incomplete_series_markets
      ?? q.excluded_incomplete_series_markets,
  ) ?? 0;
  const allCharted = validated > 0 && charted === validated;
  const text = validated
    ? `${fmt.format(charted)} / ${fmt.format(validated)} validated markets charted`
    : `${fmt.format(charted)} charted markets`;
  const detail = excluded
    ? `${fmt.format(excluded)} SQL-admitted markets excluded because the full 5m path is incomplete`
    : (allCharted ? "full validated set is charted" : "not all validated markets are charted");
  return { validated, charted, excluded, allCharted, text, detail };
}
...
function renderAllBacktestsSummary(rows) {
  const perf = backtestPerformanceSummary();
  const activeSummary = state.workflow.active_backtest?.summary || state.workflow.backtest?.summary || {};
  const coverage = perf.coverage || backtestCoverageSummary();
  ...
  const chartScopeText = rows.length === coverage.charted
    ? `${coverage.text}; ${coverage.detail}.`
    : `Current filter shows ${fmt.format(rows.length)} of ${fmt.format(coverage.charted)} charted markets; full replay totals still use ${fmt.format(coverage.validated)} validated markets.`;
  const signalItems = [
    ["Full chart set", coverage.text],
    ["Shown by filter", fmt.format(rows.length)],
    ["Buy markets shown", fmt.format(boughtRows.length)],
    ["Total fills", fmt.format(perf.buyCount)],
    ["Wins", boughtRows.length ? `${fmt.format(wonRows.length)} / ${fmt.format(boughtRows.length)}` : "--"],
    ["Shown P&L", formatSignedMoney(chartPnl)],
    ["Shown return", formatPercent(chartRoi)],
  ...
  byId("backtestSummary").innerHTML = `
    <div class="decision-block decision-main">
      ...
      <span>${escapeHtml(`${fmt.format(perf.buyCount)} fills | ${formatPercent(perf.totalRoi)} return | ${coverage.text}`)}</span>
    </div>
    <div class="decision-block decision-rule">
...
function renderAllBacktestsChart() {
  ...
  const noteRows = [
    ...
    ["Shown markets", fmt.format(rows.length)],
    ["Full charted set", coverage.text],
    ["Buy markets", fmt.format(boughtRows.length)],
    ["Total fills", fmt.format(perf.buyCount)],
    ["Won buys", boughtRows.length ? `${fmt.format(wonRows.length)} (${formatPercent(wonRows.length / boughtRows.length)})` : "--"],
    ["Shown P&L", formatSignedMoney(chartPnl)],
    ["Shown return", formatPercent(chartCost > 0 ? chartPnl / chartCost : null)],
...
function rowHasDisplayedOutcomeOdds(row) {
  if (!row || isActionOnlyPaperOddsRow(row)) return false;
  return metricNumber(row.up) !== null
    || metricNumber(row.down) !== null
    || rowHasStickyPolymarketOdds(row)
    || rowHasUsableOutcomeOdds(row)
    || rowHasDisplayPolymarketBook(row);
}

function outcomeOddsPairFromRow(row) {
  if (!row) return { up: null, down: null };
  const directUp = metricNumber(row.up);
  const directDown = metricNumber(row.down);
  return completeOutcomeOdds({
    up: directUp !== null ? directUp : outcomeProbabilityFromRow(row, "up"),
    down: directDown !== null ? directDown : outcomeProbabilityFromRow(row, "down"),
  });
}

function outcomeOddsRowUsable(row) {
  return rowHasDisplayedOutcomeOdds(row);
}
...
function rememberOutcomeOddsForWindow(anchor, candidates = []) {
  ...
  const directOddsRows = currentMarketOddsRows(rows);
  ...
  const directOdds = outcomeOddsFromCandidates(directOddsRows);
  const { up, down } = completeOutcomeOdds(preferBookOdds(bookOdds, directOdds), null);
  if (up === null && down === null) return cachedOutcomeOddsForMarket(anchor) || null;
  const sourceRows = oddsRows.length ? oddsRows : directOddsRows;
  const timestamps = sourceRows.map(outcomeOddsTimestampMicro).filter((value) => value !== null);
  const incomingTime = timestamps.length ? Math.max(...timestamps) : 0;
  const existing = keys.map((cacheKey) => state.latestOutcomeOddsByWindow.get(cacheKey)).find(rowHasDisplayedOutcomeOdds) || {};
  ...
  const source = sourceRows[0] ? { ...sourceRows[0] } : null;
  const odds = {
    ...existing,
...
function stickyOutcomeOddsForMarket(market, odds) {
  ...
  const directPrevious = keys
    ...
    .find(rowHasDisplayedOutcomeOdds) || null;
...
function renderPaperDecisionGraph(options = {}) {
  ...
  if (useUPlot) {
    ...
    const currentDisplaySample = uplotHeadSample;
    ...
    const compactInfoRows = compact
      ...
      : "";
    const latestTitle = [
      hasTruthHead ? `latest ${Math.round(uplotHeadSample.elapsedSeconds)}s in` : "waiting for Chainlink",
      hasTruthHead ? truthLabel : "Chainlink",
      hasTruthHead ? `BTC ${formatBookMoney(uplotHeadSample.btcPrice)}` : "BTC waiting",
      hasTruthHead ? formatDollarMove(uplotHeadSample.dollarMove) : "move waiting",
      hasTruthHead && uplotHeadSample.source === "chainlink" && latestRaw.receive_time_micro
...
function renderStatus() {
  ...
  byId("statusText").textContent = `${coverage.text} | ${fmt.format(activeBuys)} fills | ROI ${formatPercent(activeRoi)} | ${workflowAgeText()}${makerText}`;
}
...
function trimLiveBtcPointsForKey(key, points) {
  ...
  if (points.length <= maxRows + LIVE_TICK_STORE_TRIM_SLACK) {
    return points;
  }
...
function recomputeLiveTickDistances(market) {
  ...
  liveTickReadStorageKeysForMarket(market).forEach((key) => {
...
function rememberBackendStreamPoints(market, allPoints) {
  ...
  appendLiveBtcPoints(market || truthPoints[0] || externalPoints[0], [...truthPoints, ...externalPoints]);
